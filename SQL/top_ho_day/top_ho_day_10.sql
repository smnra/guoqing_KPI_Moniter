SELECT sdatetime,
       (CASE
                   WHEN ( area = 'BaojiFDD' ) THEN '宝鸡FDD'
                   WHEN ( area = 'BaojiTDD' ) THEN '宝鸡TDD'
                   WHEN ( area = 'XianFDD' ) THEN '西安FDD'
                   WHEN ( area = 'XianTDD' ) THEN '西安TDD'    
                   WHEN ( area = 'XianyangFDD' ) THEN '咸阳FDD'
                   WHEN ( area = 'YananTDD' ) THEN '延安FDD' 
                   WHEN ( area = 'YulinTDD' ) THEN '榆林FDD'                   
                   ELSE NULL
               END) as area,
       enb_id,
       enb_cell,
       enb_id || (SUBSTR(enb_cell, -1)) || 1 as CELLID,
      ...
      ,
       ...
       decode((CASE WHEN bts_version='FL16' THEN 'FL16' WHEN bts_version='FLF16' THEN 'FL16' WHEN bts_version='FL16A' THEN 'FL16' WHEN bts_version='TL16' THEN 'FL16' ELSE bts_version END),'FL16',sum(M8006C258-M8016C11),sum(M8006C15 - M8016C11)) as "重定向请求次数_NoCSFB",
       decode((CASE WHEN bts_version='FL16' THEN 'FL16' WHEN bts_version='FLF16' THEN 'FL16' WHEN bts_version='FL16A' THEN 'FL16' WHEN bts_version='TL16' THEN 'FL16' ELSE bts_version END),'FL16',sum(M8006C255+M8006C258+M8006C6+M8006C7+M8006C260),sum(M8006C10 + M8006C15 + M8006C6 + M8006C7 +
       M8006C180)) as "E-RAB正常释放次数",
              decode((CASE WHEN bts_version='FL16' THEN 'FL16' WHEN bts_version='FLF16' THEN 'FL16' WHEN bts_version='FL16A' THEN 'FL16' WHEN bts_version='TL16' THEN 'FL16' ELSE bts_version END),'FL16',(decode((sum(M8006C255)+sum(M8006C258)+sum(M8006C6)+sum(M8006C7)+sum(M8006C260)),0,0,round((sum(M8006C258) - sum(M8016C11))/(sum(M8006C255)+sum(M8006C258)+sum(M8006C6)+sum(M8006C7)+sum(M8006C260))*100,2))), (decode((sum(M8006C10) + sum(M8006C15) + sum(M8006C6) + sum(M8006C7) +
              sum(M8006C180)),
              0,
              0,
  ...
  FROM (SELECT M8013.sdatetime ,
          ...
          FROM (select to_char(period_start_time, 'yyyymmdd') sdatetime,
               ...
               (select to_char(period_start_time, 'yyyymmdd') sdatetime,
                   ...
                   from NOKLTE_PS_LEPSB_lncel_day PMRAW
                  where
               ...
               (select to_char(period_start_time, 'yyyymmdd') sdatetime,
                  ...
                  from NOKLTE_PS_LRRC_lncel_day PMRAW
               ...
               (select to_char(period_start_time, 'yyyymmdd') sdatetime,
                  ...
                  from NOKLTE_PS_LIANBHO_lncel_day PMRAW
                 where period_start_time >= to_date(&start_datetime, 'yyyymmdd')
                   and period_start_time < to_date(&end_datetime, 'yyyymmdd')
                 group by to_char(period_start_time, 'yyyymmdd'),
                          LNCEL_ID,
               ...
               (select to_char(period_start_time, 'yyyymmdd') sdatetime,
                   ...
                   from NOKLTE_PS_LCELLR_lncel_day PMRAW
                  where
               ...
               (select to_char(period_start_time, 'yyyymmdd') sdatetime,
                   ...
                   from NOKLTE_PS_LCELLT_lncel_day PMRAW
                  where
               ...
               (select to_char(period_start_time, 'yyyymmdd') sdatetime,
                      ...
                      ,
                       ...
                       c.version  bts_version,
                       c.city||c.netmodel as area,
                       Trim(lnbts.co_name) bts_name,
                       Trim(lncel.co_name) cel_name
                  from NOKLTE_PS_LUEST_lncel_day PMRAW,
                 ...
                 where period_start_time >= to_date(&start_datetime, 'yyyymmdd')
                   ...
                   and c.lncel_objid = PMRAW.LNCEL_ID
               ...
               (select to_char(period_start_time, 'yyyymmdd') sdatetime,
                   ...
                   from NOKLTE_PS_LIENBHO_lncel_day PMRAW
                  where
               ...
               (select to_char(period_start_time, 'yyyymmdd') sdatetime,
                  ...
                  from NOKLTE_PS_LNCELHO_lncel_day PMRAW
                 where period_start_time >= to_date(&start_datetime, 'yyyymmdd')
                   and period_start_time < to_date(&end_datetime, 'yyyymmdd')
                 group by to_char(period_start_time, 'yyyymmdd'),
                          LNCEL_ID,
               ...
               (select to_char(period_start_time, 'yyyymmdd') sdatetime,
                  ...
                  from NOKLTE_PS_LISHO_lncel_day PMRAW
                 where period_start_time >= to_date(&start_datetime, 'yyyymmdd')
                   and period_start_time < to_date(&end_datetime, 'yyyymmdd')
                 group by to_char(period_start_time, 'yyyymmdd'),
                          LNCEL_ID,
               ...
               (select to_char(period_start_time, 'yyyymmdd') sdatetime ,
                  ...
                  from NOKLTE_PS_LCELAV_LNCEL_day PMRAW
                 where period_start_time >= to_date(&start_datetime, 'yyyymmdd')
                   and period_start_time < to_date(&end_datetime, 'yyyymmdd')
                 group by to_char(period_start_time, 'yyyymmdd'),
                          MRBTS_ID,
 ...
 GROUP BY sdatetime,
          ...
          (CASE
                   ...
                   ELSE NULL
               END)
having  Round(decode(sum(M8014C6), 0, 0, 100 * sum(M8014C7) / sum(M8014C6)),
             2) <=80 and  
             (sum(M8014C6) - sum(M8014C7)) <50
             and (sum(M8014C6) - sum(M8014C7)) >=10