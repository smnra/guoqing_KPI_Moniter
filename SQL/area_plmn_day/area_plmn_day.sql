...
FROM
    ...
    (SELECT 
        to_char(tab2.pm_date,'yyyymmdd') DDATE, 
        tab2.区域,
    ...
    FROM 
	    ...
	    luest.SIGN_CONN_ESTAB_ATT_EMG + decode(luest.SIGN_CONN_ESTAB_ATT_DEL_TOL,'',0, luest.SIGN_CONN_ESTAB_ATT_DEL_TOL)+ 
	    decode(luest.SIGN_CONN_ESTAB_ATT_HIPRIO,'',0,luest.SIGN_CONN_ESTAB_ATT_HIPRIO))), 0, 0, sum(luest.SIGN_CONN_ESTAB_COMP) / 
	    ...
	    luest.SIGN_CONN_ESTAB_ATT_HIPRIO)),sum(luest.SIGN_CONN_ESTAB_ATT_MO_S + luest.SIGN_CONN_ESTAB_ATT_MT +
	    ...
	    decode(luest.SIGN_CONN_ESTAB_ATT_HIPRIO,'',0,luest.SIGN_CONN_ESTAB_ATT_HIPRIO))) AS RRC连接建立请求次数,
            ...
            decode((CASE WHEN c.version='FL16' THEN 'FL16' WHEN c.version='FLF16' THEN 'FL16' WHEN c.version='FL16A' THEN 'FL16' WHEN c.version='TL16' THEN 'FL16' ELSE c.version END),'FL16',sum(lepsb.EPS_BEARER_SETUP_ATTEMPTS - lepsb.EPS_BEARER_SETUP_COMPLETIONS - lepsb.ERAB_INI_SETUP_FAIL_RNL_UEL - lepsb.ERAB_ADD_SETUP_FAIL_RNL_UEL - 
    ...
    (SELECT 
        to_char(tab5.pm_date,'yyyymmdd') DDATE, 
        tab5.区域,
    ...
    (SELECT 
        to_char(tab6.pm_date,'yyyymmdd') DDATE, 
        tab6.区域,
    ...
    (SELECT 
        to_char(tab7.pm_date,'yyyymmdd') DDATE, 
        tab7.区域,
    ...
    (SELECT 
        to_char(tab8.pm_date,'yyyymmdd') DDATE, 
        tab8.区域,
    ...
    (SELECT 
        to_char(tab10.pm_date,'yyyymmdd') DDATE, 
        tab10.区域,
    ...
    GROUP BY
        to_char(tab10.pm_date,'yyyymmdd'), 
        tab10.区域
    ...
    (SELECT 
        to_char(tab11.pm_date,'yyyymmdd') DDATE, 
        tab11.区域,
    ...
    GROUP BY
        to_char(tab11.pm_date,'yyyymmdd'), 
        tab11.区域
...
WHERE
    ...
    AND tab1.DDATE = tab6.DDATE AND tab1.区域 = tab6.区域
    AND tab1.DDATE = tab7.DDATE AND tab1.区域 = tab7.区域
    AND tab1.DDATE = tab8.DDATE AND tab1.区域 = tab8.区域
    AND tab1.DDATE = tab9.DDATE AND tab1.区域 = tab9.区域
    AND tab1.DDATE = tab10.DDATE AND tab1.区域 = tab10.区域
    AND tab1.DDATE = tab11.DDATE AND tab1.区域 = tab11.区域
