SELECT to_char(tab1.pm_date, 'yyyymmdd') DDATE,
 ...
 group by to_char(tab1.pm_date, 'yyyymmdd'),
          ...
          基站数,
          小区数
having round(decode(sum(tab3.LTE业务释放次数), 0, 0, sum(tab3.LTE业务掉线次数) / sum(tab3.LTE业务释放次数)) * 100, 2) >= 0.5 and round(decode(sum(tab3.LTE业务释放次数), 0, 0, sum(tab3.LTE业务掉线次数) / sum(tab3.LTE业务释放次数)) * 100, 2) < 1.5 and sum(tab3.LTE业务掉线次数) > = 100
 order by DDATE
